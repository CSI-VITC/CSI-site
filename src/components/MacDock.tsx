...
function DockItem({ label, iconSrc, bg, padding, onClick, mouseX }: DockItemProps) {
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="dock-item"
      >
        <div className="icon-inner" style={{ background: bg || "transparent", padding: padding || "0" }}>
...
export default function MacDock({ onOpen }: { onOpen: (id: string) => void }) {
  ...
  const items: Array<{ id: string; label: string; iconSrc: string; bg?: string; padding?: string }> = [
    ...
    { id: "projects", label: "Projects", iconSrc: "/icons/VSCode.svg", padding: "4px" },
